const progressBarcontext = document.querySelector('.progress-bar-context');
const scrollableContentcontext = document.querySelector('.scrollable-content-context');

const BLOCK_WIDTH = 500;
const BLOCKS_COUNT = 13;
const blockWidthInPersent = 90 / BLOCKS_COUNT;
const DEFAULT_OPACITY = 0.5;
const ACTIVE_OPACITY = 1;

const boxescontext = Array.from(scrollableContentcontext.children);

scrollableContentcontext.addEventListener('scroll', function() {
    const winScroll = scrollableContentcontext.scrollLeft;
    const width = scrollableContentcontext.scrollWidth - scrollableContentcontext.clientWidth;
    const scrolled = (winScroll / width) * 100;
    const childrenIndex = Math.floor(scrolled / blockWidthInPersent)
    document.getElementById("myBar-context").style.width = scrolled + "%";
    const activeChildren = boxescontext[childrenIndex];
    if (activeChildren) {
        boxescontext.forEach((children) => {
...
function wheelHandler (e) {
    e.preventDefault();
    e.stopPropagation();
    scrollableContentcontext.scrollLeft += e.deltaY;
}

scrollableContentcontext.addEventListener('mouseenter', (e) => {
    document.body.style.overflowY = 'hidden';
    document.addEventListener('wheel', wheelHandler)
})

scrollableContentcontext.addEventListener('mouseleave', (e) => {
